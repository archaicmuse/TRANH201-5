...
class GUI(Gtk.Application):
	...
	def open_settings(self, *args):
		...
		self.swin.connect("delete-event", lambda x: self.swin.destroy())
		...
		extension_label = Gtk.Label("Extension : ")
		self.extension_entry = Gtk.Entry()
		self.extension_entry.set_text(cnfg.get("database" , "extension"))
		hbox1.pack_start(extension_label, False, False, 0)
		hbox1.pack_end(self.extension_entry, False, False, 0)

		hbox2 = Gtk.HBox(spacing=5)
		database_folder_label = Gtk.Label("Database folder : ")
		self.database_folder_entry = Gtk.Entry()
		self.database_folder_entry.connect("insert-at-cursor", self.choose_folder)
		self.database_folder_entry.set_text(cnfg.get("database","folder"))
		hbox2.pack_start(database_folder_label, False, False, 0)
		hbox2.pack_end(self.database_folder_entry, False, False, 0)

		hbox3 = Gtk.HBox(spacing=5)
		gallery_folder_label = Gtk.Label("Gallery folder : ")
		self.gallery_folder_entry = Gtk.Entry()
		self.gallery_folder_entry.connect("toggle-overwrite", self.choose_folder)
		self.gallery_folder_entry.set_text(cnfg.get("gallery","folder"))
		hbox3.pack_start(gallery_folder_label, False, False, 0)
		hbox3.pack_end(self.gallery_folder_entry, False, False, 0)

		hbox4 = Gtk.HBox(spacing=5)
		temperature_label = Gtk.Label("Temperature unit : ")
		self.temperature_entry = Gtk.Entry()
		self.temperature_entry.set_text(cnfg.get("gallery","units"))
		hbox4.pack_start(temperature_label, False, False, 0)
		self.temperature_entry.set_max_length(1)
		self.temperature_entry.set_max_width_chars(1)
		self.temperature_entry.set_width_chars(1)
	...
	def save_settings(self, *args):
		...
		config.set("database","folder", self.database_folder_entry.get_text())
		config.set("gallery","folder", self.gallery_folder_entry.get_text())
		config.set("database","extension",self.extension_entry.get_text())
		config.set("arduino","xpixel", str(int(self.xpixel_entry.get_text())))
		config.set("arduino","ypixel", str(int(self.ypixel_entry.get_text())))
		config.set("gallery", "units", self.temperature_entry.get_text().upper())
